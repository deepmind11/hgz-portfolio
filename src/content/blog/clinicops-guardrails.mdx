...
That's correct. It will handle "which patients have unreported NorthStar Response samples", "what's the average coverage of passed samples", "list failed pipeline runs this week" — all of it, in milliseconds, for about $0.0003 per call. The NL-to-SQL problem is effectively solved by any reasonable frontier model in 2026.
...
Third, a **narrator** converts the raw result rows back into a human sentence. This is where you get "5 samples are currently stuck in QC, all from the NorthStar Select assay, with the oldest dating back 8 days." The narrator is also responsible for flagging surprising answers ("this is twice the usual count").
